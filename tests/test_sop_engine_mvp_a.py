...
from Action_Detection_SOP.sop_engine import HelmetRuleConfig, SessionizationConfig, SopEngine, SopEngineConfig, StepStatus
        ...
        *,
        ...
        for _ in range(present_frames):
            ...
            res = engine.update(
                ...
                frame_idx=frame_idx,
                persons=[_person_det()],
                helmets=[_helmet_det()] if helmet_present else [],
            )
            if res is not None:
                results.append(res)
            frame_idx += 1
        ...
        for _ in range(absent_frames):
            t_s = frame_idx / analysis_fps
            res = engine.update(time_s=t_s, frame_idx=frame_idx, persons=[], helmets=[])
            if res is not None:
                results.append(res)
            frame_idx += 1
        return results
